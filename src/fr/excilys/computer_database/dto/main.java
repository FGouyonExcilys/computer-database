		...
		try {
			...
			while (menu == 0) {
				...
				case "3":
					...
					System.out.println("\n" + computerDao.afficherInfoComputer(Integer.parseInt(strDetails)) + "\n");
					break;
				...
				case "5":
					...
					computerDao.modifier(
							new Computer(Integer.parseInt(strIdModif), strNameModif, Date.valueOf(strIntroducedModif),
									Date.valueOf(strDiscontinuedModif), new Company(Integer.parseInt(strCompanyIdModif), "") ));
			...
			/*
			 * for (Company c : companyDao.lister()) { System.out.println(c.toString() +
			 * "\n"); }
			 * 
			 * for (Computer c : computerDao.lister()) { System.out.println(c.toString() +
			 * "\n"); }
			 * 
